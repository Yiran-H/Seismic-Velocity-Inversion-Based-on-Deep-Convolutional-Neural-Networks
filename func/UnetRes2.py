...
@citation: fangshuyang (yfs2016@hit.edu.cn)
@adding residual net: Yiran Huang (yrhuang@bu.edu)
...
class unetConv2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        cat = self.do1(outputs)

        outputs = self.conv2(outputs)

        outputs = self.bn3(self.conv3(outputs))
        outputs = self.relu3(torch.add(outputs, cat))
        return outputs
